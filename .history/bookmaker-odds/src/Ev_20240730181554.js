  ...
  useEffect(() => {
    const fetchGames = async () => {
      const redisClient = new Redis({
        ...
        port: 6379,
        username: "red-cqbvt1jv2p9s73fe92c0",
        password: "SabB5CAuaIOF4UOKMz2bGn7c8VZ2wVLI",
        tls: {
          minVersion: "TLSv1.2",
        },
      });
      ...
      try {
        const gameData = await redisClient.get("game_data:combined*");
        if (gameData) {
          const parsedData = JSON.parse(gameData);
          setGames(Object.values(parsedData));
        }
      } catch (error) {
