...
const EvCalculator = () => {
  ...
  useEffect(() => {
    const fetchGames = async () => {
      try {
        const response = await fetch("https://bettingscraperodds.onrender.com/data");
  ...
  useEffect(() => {
    if (selectedGame && selectedBookmaker && selectedMarket) {
      calculateEV();
    }
  }, [selectedGame, selectedBookmaker, selectedMarket, stake]);

  const calculateEV = () => {
    const gameData = games.find(game => game.home + ' vs ' + game.away === selectedGame);
    if (!gameData) return;

    const odds = gameData.urls[selectedMarket][selectedBookmaker];
    const impliedProbabilities = Object.values(odds).map(odd => 1 / odd);
    const totalImpliedProbability = impliedProbabilities.reduce((a, b) => a + b, 0);
    
    const results = {};
    Object.entries(odds).forEach(([key, odd]) => {
      const fairProbability = (1 / odd) / totalImpliedProbability;
      const ev = (odd * stake * fairProbability) - stake;
      results[key] = {
        odd,
        ev: ev.toFixed(2),
        evPercentage: ((ev / stake) * 100).toFixed(2)
      };
    });
  ...
  const getAvailableBookmakers = () => {
    if (!selectedGame || !selectedMarket) return [];
    const gameData = games.find(game => game.home + ' vs ' + game.away === selectedGame);
    return gameData ? Object.keys(gameData.urls[selectedMarket]) : [];
  };

  const getAvailableMarkets = () => {
    if (!selectedGame) return [];
    const gameData = games.find(game => game.home + ' vs ' + game.away === selectedGame);
    return gameData ? Object.keys(gameData.urls) : [];
  };
